...
void do_aifaction( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !str_cmp( arg1, "create" ) )
    {
        ...
        /* --- find a good location and spawn starter base --- */
        int placed = 0;
        bool found_loc = ai_find_faction_location( &bx, &by, &bz );
            ...
            if ( hq )
            {
                ...
                if ( ai_find_free_spot( BUILDING_TURRET, bx, by, bz, 3, &tx, &ty ) )
                    if ( ai_place_building( fac, BUILDING_TURRET, tx, ty, bz ) )
                        placed++;
                if ( ai_find_free_spot( BUILDING_TURRET, bx, by, bz, 4, &tx, &ty ) )
                    if ( ai_place_building( fac, BUILDING_TURRET, tx, ty, bz ) )
                        placed++;
            }
        }
        ...
        save_ai_factions();
        if ( found_loc && placed > 0 )
            snprintf( buf, sizeof(buf),
                      "AI faction '%s' created with %d building(s) at %d/%d.\n\r",
                      fac->name, placed, bx, by );
        else
            snprintf( buf, sizeof(buf),
                      "AI faction '%s' created. Could not auto-place a base "
                      "(map too full?). Use bset to assign buildings manually.\n\r",
                      fac->name );
        send_to_char( buf, ch );
        return;
    }
